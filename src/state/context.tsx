import { PropsWithChildren, createContext, useReducer, useContext } from "react";
...
const userActions = {
  ...
  const userReducer = (state: User | null | undefined, action: UserActionType) => {
    switch (action.type) {
  ...
  const initialState: ContextType = {
    user:  localStorage.getItem(LocalStorageKeys.user) ? JSON.parse(localStorage.getItem(LocalStorageKeys.user) || "") as User : null ,
  };
  ...
  const mainReducer = (state: ContextType, action: UserActionType): ContextType => {
    return {
      ...state,
